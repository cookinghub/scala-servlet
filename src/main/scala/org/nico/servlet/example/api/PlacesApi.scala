...
/**
  ...
  */
object PlacesApi extends ActionController {
  ...
  /**
    ...
    */
  def createPlace = SecuredAction { implicit request =>
    bodyAs[CreatePlace].fold(badRequest => badRequest, { body =>
      val created = Places.create(body)
      Ok(created)
    })
  }

  /**
    * @return Ok if matching plase, NotFound if no match, BadRequest otherwise
    */
  def getPlace = SecuredAction { implicit request =>
    paramAs[String]("placeId").map { placeId =>
      Places.findOne(placeId).map(p => Ok(p)).getOrElse(NotFound())
